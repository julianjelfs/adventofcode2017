...
removeCommas ws = fmap (filter (\c -> c /= ',')) ws
...
lineToNode (name:weight) = Node name (read (head weight) :: Int) []
...
findNode nodes node = head $ filter ((==) node) nodes

isRoot :: [Node] -> Node -> Bool
isRoot nodes node = (length $ filter (isChildOf node) nodes) == 0

getRoot nodes = head $ filter (isRoot nodes) nodes
...
replaceChild nodes node = buildTree nodes (findNode nodes node)

totalWeight :: Node -> Int
totalWeight (Node _ w children) = w + (sum $ (fmap totalWeight children))

findWonkyNode adjustBy (Node n w []) = w - adjustBy
findWonkyNode adjustBy (Node n w c) =
  let childWeights = zip (fmap totalWeight c) c
      (min, max) =
        ( minimumBy (comparing fst) childWeights
        , maximumBy (comparing fst) childWeights)
      (minw, maxw) = (fst min, fst max)
  in if minw /= maxw
       then findWonkyNode (maxw - minw) (snd max)
       else w - adjustBy
